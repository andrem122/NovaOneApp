...
class AppointmentsContainerViewController: UIViewController, NovaOneObjectContainer {
    ...
    func saveObjectsToCoreDataAndSend(to objectsTableViewController: UITableViewController, objects: [Decodable]) {
        ...
        guard let entity = NSEntityDescription.entity(forEntityName: Defaults.CoreDataEntities.appointment.rawValue, in: PersistenceService.context) else { return }
            ...
            guard let appointmentsTableViewController = objectsTableViewController as? AppointmentsTableViewController else { return }
            appointmentsTableViewController.objects = coreDataAppointments
            appointmentsTableViewController.filteredObjects = coreDataAppointments
    }
                                ...
                                switch result {
                                    ...
                                    case .success(let appointments):
                                        ...
                                        UIHelper.showSuccessContainer(for: self, successContainerViewIdentifier: Defaults.SplitViewControllerIdentifiers.appointments.rawValue, containerView: self?.containerView ?? UIView(), objectType: UISplitViewController.self) {
                                            ...
                                            guard
                                                let appointmentsSplitViewController = appointmentsSplitViewController as? UISplitViewController,
                                                let appointmentsNavigationController = appointmentsSplitViewController.viewControllers.first as? UINavigationController,
                                                let appointmentsTableViewController = appointmentsNavigationController.viewControllers.first as? AppointmentsTableViewController
                                            else { return }
                                            
                                            // Save data in CoreData
                                            self?.saveObjectsToCoreDataAndSend(to: appointmentsTableViewController, objects: appointments)
