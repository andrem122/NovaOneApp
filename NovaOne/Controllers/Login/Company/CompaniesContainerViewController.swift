...
class CompaniesContainerViewController: UIViewController, NovaOneObjectContainer {
    ...
    func saveObjectsToCoreDataAndSend(to objectsTableViewController: UITableViewController, objects: [Decodable]) {
        ...
        guard let entity = NSEntityDescription.entity(forEntityName: Defaults.CoreDataEntities.company.rawValue, in: PersistenceService.context) else { return }
            ...
            // Send the data to the objects view controller
            let sortDescriptors = [NSSortDescriptor(key: "id", ascending: false)] // Sort data by id
            let coreDataCompanies = PersistenceService.fetchEntity(Company.self, filter: nil, sort: sortDescriptors)
            
            guard let companiesTableViewController = objectsTableViewController as? CompaniesTableViewController else { return }
            companiesTableViewController.objects = coreDataCompanies
            companiesTableViewController.filteredObjects = coreDataCompanies
    }
                                    ...
                                    switch result {
                                        ...
                                        case .success(let companies):
                                            ...
                                            UIHelper.showSuccessContainer(for: self, successContainerViewIdentifier: Defaults.SplitViewControllerIdentifiers.companies.rawValue, containerView: self?.containerView ?? UIView(), objectType: UISplitViewController.self) {
                                                ...
                                                guard
                                                    let companiesSplitViewController = companiesSplitViewController as? UISplitViewController,
                                                    let companiesNavigationController = companiesSplitViewController.viewControllers.first as? UINavigationController,
                                                    let companiesTableViewController = companiesNavigationController.viewControllers.first as? CompaniesTableViewController
                                                else { return }
                                                
                                                // Save data in CoreData
                                                self?.saveObjectsToCoreDataAndSend(to: companiesTableViewController, objects: companies)
